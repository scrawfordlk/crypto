...
def encode(data: bitarray):
    ...
    encoded_length = (
        data_length + num_parity_bits + 1
    )  # need +1 for the overall parity bit
    ...
    for encoded_index in range(
        3, len(encoded)
    ):  # start at 3 instead of 1 to skip first two parity bits
...
def decode(encoded: bitarray):
    ...
    )  # -1 to remove overall parity bit from length, +1 since first parity bit at len 1 (2 ** 0)
    ...
    if index_of_error and overall_correct:  # two errors found
        ...
        index_of_error and not overall_correct
...
def bytes_to_bits(byte_stream: bytearray):
    ...
    for byte in byte_stream:
        ...
        for bit in data:
            out.append(
                0 if bit == "0" else 1
            )  # note that all bits go to 1 if we just append bit, since it's a non-null string
    return out
